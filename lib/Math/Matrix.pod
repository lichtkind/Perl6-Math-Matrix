...
This repo will be the new home and successor of L<this project|https://github.com/pierre-vigier/Perl6-Math-Matrix>.
...
=item B<L<accessors|#accessors>>: L<element|#element>, L<AT-POS|#at-pos>, L<row|#row>, L<column|#column>, L<diagonal|#diagonal>, L<skew-diagonal|#skew-diagonal>, L<submatrix|#submatrix>
=item B<L<converter|#converter>>: L<Bool|#bool>, L<Str|#str>, L<Numeric|#numeric>, L<Range|#range>,
      L<Array|#array>, L<list|#list>, L<list-rows|#list-rows>, L<list-columns|#list-columns>, L<Hash|#hash>, L<gist|#gist>, L<perl|#perl>
=item B<L<boolean properties|#boolean-properties>>: L<square|#is-square>, L<zero|#is-zero>, L<identity|#identity>, L<triangular|#is-triangular>, 
      L<diagonal|#is-diagonal>, L<-dominant|#is-diagonally-dominant>, L<-constant|#is-diagonal-constant>, L<catalecticant|#is-catalecticant>,
...
=item B<L<numeric properties|#numeric-properties>>: L<size|#size>, L<density|#density>, L<bandwith|#bandwith>, L<trace|#trace>, L<rank|#rank>, L<nullity|#nullity>,
      L<determinant|#determinant>, L<minor|#minor>, L<norm|#norm>, L<condition|#condition>, L<element-type|#element-type>
=item B<L<derived matrices|#derived-matrices>>: L<transposed|#transposed>, L<negated|#negated>, L<conjugated|#conjugated>, L<adjugated|#adjugated>, L<inverted|#inverted>,
      L<reduced-row-echelon-form|#reduced-row-echelon-form>
=item B<L<decompositions|#decompositions>>: L<LUCrout|#decompositionlucrout>, L<LU|#decompositionlu>, L<Cholesky|#decompositioncholesky>
...
=item B<L<list like ops|#list-like-operations>>: L<elems|#elems>, L<elem|#elem>, L<cont|#cont>, L<map|#map>, L<map-with-index|#map-with-index>, 
      L<reduce|#reduce>, L<reduce-rows|#reduce-rows>, L<reduce-columns|#reduce-columns>
=item B<L<structural ops|#structural-operations>>: L<move-row|#move-row>, L<move-column|#move-column>, L<swap-rows|#swap-rows>, L<swap-columns|#swap-columns>,
...
This method is a constructor, that returns a zero matrix (sometimes called empty),
as checked by L<is-zero|#is-zero>. It has the L<size|#size> as given by arguments.
If only one argument is given, the matrix is L<quadratic|#is-square>. All the L<element|#element>s are set to 0.
...
This method is a constructor that returns an identity matrix (as checked by L<is-identity|#is-identity>)
of the size given in the only and required parameter.
All the L<element|#element>s are set to 0 except the top/left to bottom/right diagonale is set to 1.
...
This method is a constructor that returns an diagonal matrix (as checked by L<is-diagonal|#is-diagonal>)
of the size given by count of the parameter.
All the L<element|#element>s are set to 0 except the top/left to bottom/right diagonal, set to given values.
...
the matrix product (method L<dot-product|#dot-product>, or operator dot) of a column vector
...
L<element|#element>, L<AT-POS|#at-pos>, L<row|#row>, L<column|#column>, L<diagonal|#diagonal>, L<skew-diagonal|#skew-diagonal>, L<submatrix|#submatrix>

=head3 L<element|#accessors>

Gets value of one element in row (first parameter) and column (second parameter - counting always from 0).
Sometimes its called matrix cell, to ditinct from other type of elements.
...
Without an argument it returns values of main diagonal elements as a list. 
Use the optional parameter to get any other parallel diagonal. 
Positive value for the ones above - negative below and 0 for the main diagonal.
...
Unlike a I<diagonal>, a skew diagonal is only defined for L<square|#is-square> matrixes.
It runs from $matrix[n][0] to $matrix[0][n], n being row or column size - 1.
Use the optional parameter to get any other parallel skew diagonal. 
...
Returns a matrix that might miss certain rows and columns of the original.
...
In mathematics, a submatrix is built by leaving out one row and one column.
In the two argument format you name these by their index ($row, $column).
...
that contains only the requested rows and columns - still in the original order.
...
Methods that convert a matrix into other types: L<Bool|#bool>, L<Str|#str>, L<Numeric|#numeric>, L<Range|#range>, L<Array|#array>, L<Hash|#hash>, L<list|#list>,
L<list-rows|#list-rows>, L<list-columns|#list-columns> or allow different views on the overall content (output formats): L<gist|#gist>, L<perl|#perl>.
...
Thus, the return value is a always of boolean type.
Arguments, like in case of L<is-diagonally-dominant|#is-diagonally-dominant>,
are only necessary when a method can tell you about a group of closely related properties.

L<square|#is-square>, L<zero|#is-zero>, L<identity|#identity>,
L<triangular|#is-triangular>, L<upper-triangular|#is-upper-triangular>, L<lower-triangular|#is-lower-triangular>,
L<diagonal|#is-diagonal>, L<diagonally-dominant|#is-diagonally-dominant>, L<diagonal-constant|#is-diagonal-constant>, L<catalecticant|#is-catalecticant>,
L<symmetric|#is-symmetric>, L<anti-symmetric|#is-antisymmetric>, L<unitary|#is-unitary>, L<self-adjoint|#is-self-adjoint>, L<invertible|#is-invertible>,
L<orthogonal|#is-orthogonal>, L<positive-definite|#is-positive-definite>, L<positive-semidefinite|#is-positive-semidefinite>
...
True if every L<element|#element> on the main L<diagonal|#diagonal> (where row index equals column index) is 1
and any other element is 0.
...
True if matrix  I<is-upper-triangular> or I<is-lower-triangular> (none - strict).


=head4 L<is-upper-triangular|#boolean-properties>

a.k.a I<right triangular> matrix: every L<element|#element> below the L<diagonal|#diagonal> (where row index is greater than column index) is 0.
In other words: the L<lower-bandwith|#lower-bandwith> is zero.
...
=head4 L<is-lower-triangular|#boolean-properties>

a.k.a I<left triangular> matrix: every L<element|#element> above the L<diagonal|#diagonal> (where row index is smaller than column index) is 0.
In other words: the L<upper-bandwith|#upper-bandwith> is zero.
...
True if matrix is L<square|#is-square> and only elements on the L<diagonal|#diagonal> differ from 0.
In other words: if matrix is L<upper-triangular|#is-upper-triangular> and L<lower-triangular|#is-lower-triangular>.
...
Checks if caller is a I<diagonal-constant> or I<Töplitz matrix>.
True if every L<diagonal|#diagonal> is the a collection of L<element|#element>s that hold the same value.
...
Checks if caller is a I<catalecticant> or I<Hankel matrix>.
True if every L<skew diagonal|#skew-diagonal> is the a collection of elements that hold the same value.
Catalecticant matrices are L<symmetric|#is-symmetric>.
...
Also called I<nonsingular> or I<nondegenerate>. 
(To ask if matrix is degenerate or singular - simply negate the result with ! or I<not>).
Is True if matrix (L<is-square|#is-square>) and L<determinant|#determinant> is not zero.
All rows or colums have to be independent vectors. 
Please check this before using $matrix.L<inverted|#inverted>, or you will get an exception, in case it was degenerate.
...
An unitery matrix multiplied (L<dot-product|#dot-product>) with its concjugate transposed 
derivative (.conj.T) is an L<identity|#is-identity> matrix, or said differently:
...
L<size|#size>, L<density|#density>, B<L<bandwith|#bandwith>>: (L<lower-bandwith|#lower-bandwith>, L<upper-bandwith|#upper-bandwith>), L<trace|#trace>, L<rank|#rank>, L<nullity|#nullity>,
L<determinant|#determinant>, L<minor|#minor>, L<norm|#norm>, L<condition|#condition>, B<L<element-type|#element-type>>:
(L<narrowest-element-type|#narrowest-element-type>, L<widest-element-type|#widest-element-type>)
...
matrix is returned, because the other one is its transposed.
...
Without pivotisation the marix has to be invertible (L<square|#is-square> and full L<rank|#rank>ed).
...
$L is a left triangular matrix and $R is a right one
This decomposition works only on invertible matrices (L<square|#is-square> and full L<rank|#rank>ed).
...
They are: L<equal|#equal>, L<add|#add>, L<multiply|#multiply>, L<dot-product|#dot-product>, L<tensor-product|#tensor-product>.
...
Adding a matrix, vector or scalar. Named arguments I<:row> and I<:column> have no fixed position.


=head4 L<add matrix|#add>

When adding two matrices, they have to be of the same size. Instead of
Math::matrix object you can also provide the element data as 
...
and give a list or array (which have to fit the matrix size).
...
When adding a single number to the matrix, it will be added to every L<element|#element>.
If you provide a row or column number it will be only added to that row or column.
In case you provide both, only a single element gets a different value in the result matrix.
...
this operation is the simple, scalar multiplication applied to L<element|#element>s.
That is why this method works analogous to the scalar variant of L<add|#add-scalar>.
However, when a matrix of same size is given, the result will be a matrix
...
Operators with the methods they refer to. (Most ops are just aliases.)
For more explanations of the ops with examples see naext chapter: L<ops chapter|#operators>:
