...
=head1 METHODS
=item L<constructors|#constructors>: L<new []|#new-->, L<new ()|#new---1>, L<new ""|#new---2>, L<new-zero|#new-zero>, L<new-identity|#new-identity>, L<new-diagonal|#new-diagonal>, L<new-vector-product|#new-vector-product>
=item L<accessors|#accessors>: L<cell|#cell>, L<AT-POS|#at-pos>, L<row|#row>, L<column|#column>, L<diagonal|#diagonal>, L<submatrix|#submatrix>
...
=item L<list like ops|#list-like-matrix-operations>: L<elems|#elems>, L<elem|#elem>, L<cont|#cont>, L<equal|#equal>, L<map|#map>, L<map-row|#map-row>, L<map-column|#map-column>, L<map-cell|#map-cell>, L<reduce|#reduce>, L<reduce-rows|#reduce-rows>, L<reduce-columns|#reduce-columns>
...
If you provide two ranges (row-min .. row-max, col-min .. col-max), you get the two dimensional
excerpt of the matrix that is defined by these ranges.

    say $m.submatrix(1..1, 0..*) :    3 4 5        

When provided with two lists (or arrays) of values (first for the rows - second for columns)
a new matrix will be created with that selection of rows and columns.
Please note, that you can pick any row/column in any order and as many times you prefer.
They will displayed in the order they are listed in the arguments.
...
This is the same format as expected by L<Math::Matrix.new("")|#new---2>.
...
Content of all cells as an array of arrays (same format that was put into L<Math::Matrix.new([...])|#new-->).
...
=head3 L<elem|#list-like-matrix-operations>
...
If a range is provided as argument, at least one cell value has to be 
